...
def weekly_score_file(stats: dict, date_file: str):
    flat = {
        "date": date_file,
        "n_tickets": stats["n_tickets"],
        "score_mean": stats["score_mean"],
...
def main():
    ...
    df_scored = pd.read_csv(f'week_ready_{ts}.csv', sep=";", index_col=0)
    st.write(f"Загружено строк: **{df_scored.shape[0]}**")
    ...
    if st.button('Отправить TOP20% по email'):
        send_mail(
            ...
            body=f"Больше информации на сервере SERVER в INCIDENTS.ps1 на рабочем столе.\n\n\nСписок TOP20% подозрительных инцидентов на прошлой неделе:\n{', '.join(top20.to_list())}.",
    ...
    with st.expander("Диагностика модели", expanded=False):
        ...
        st.bar_chart(df_scored.sort_values("score", ascending=True)["score"])
